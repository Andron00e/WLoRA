...
from peft import (AdaLoraConfig, BOFTConfig, HRAConfig, LoraConfig,
...
def skip_adalora_and_gpt2(test_list):
    return [
        ...
        if not (("GPT2LMHeadModel" in test[1]) and (test[2] == AdaLoraConfig))
    ]
...
def skip_boft_or_hra_and_gpt2(test_list):
    return [
        ...
        if not (
            ...
            and ((test[2] == BOFTConfig) or (test[2] == HRAConfig))
        )
    ]
...
class PeftDecoderModelTester(unittest.TestCase, PeftCommonTester):
    ...
    @parameterized.expand(
        PeftTestConfigManager.get_grid_parameters(
            FULL_GRID, filter_params_func=skip_boft_or_hra_and_gpt2
        )
    )
    ...
    def test_prompt_tuning_text_prepare_for_training(
        self, test_name, model_id, config_cls, config_kwargs
    ):
    ...
    def test_prompt_tuning_config_invalid_args(self):
        ...
        with pytest.raises(
            ValueError,
            match="tokenizer_kwargs only valid when using prompt_tuning_init='TEXT'.",
        ):
            PromptTuningConfig(
                base_model_name_or_path=model_id,
                tokenizer_name_or_path=model_id,
    ...
    def test_generate_pos_args(self, test_name, model_id, config_cls, config_kwargs):
        ...
        self._test_generate_pos_args(
            model_id, config_cls, config_kwargs, raises_err=False
        )
    ...
    @parameterized.expand(PeftTestConfigManager.get_grid_parameters(FULL_GRID))
    def test_training_decoders_layer_indexing(
        self, test_name, model_id, config_cls, config_kwargs
    ):
    ...
    def test_training_decoders_gradient_checkpointing(
        self, test_name, model_id, config_cls, config_kwargs
    ):
