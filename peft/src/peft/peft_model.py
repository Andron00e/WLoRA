...
from .tuners import (AdaLoraModel, AdaptionPromptModel, BOFTModel,  # ADD
                     FourierFTModel, HRAModel, IA3Model, LNTuningModel,
                     LoHaModel, LoKrModel, LoraModel, MultitaskPromptEmbedding,
                     OFTModel, PolyModel, PrefixEncoder, PromptEmbedding,
                     PromptEncoder, VBLoRAModel, VeraModel, WeightLoraModel,
                     XLoraConfig, XLoraModel)
from .tuners.tuners_utils import BaseTuner, BaseTunerLayer
from .utils import (SAFETENSORS_WEIGHTS_NAME,
                    TRANSFORMERS_MODELS_TO_PREFIX_TUNING_POSTPROCESS_MAPPING,
                    WEIGHTS_NAME, PeftType, TaskType, _get_batch_size,
                    _prepare_prompt_learning_config, _set_adapter,
                    _set_trainable, get_peft_model_state_dict,
                    id_tensor_storage, infer_device, load_peft_weights,
...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
        ...
        **kwargs: Any,
        ...
        def save_mutated_as_lora(
            ...
            if not any(
                ...
                for prefix in ["pissa", "olora", "true"]
            ):
                warnings.warn(
            ...
            try:
                ...
                is_olora = (
                    str(
                        self.peft_config[initial_adapter_name].init_lora_weights
                    ).lower()
                    == "olora"
                )
                ...
                if path_initial_model_for_weight_conversion is not None:
                    ...
                    if peft_config.rank_pattern:
                        peft_config.rank_pattern = {
                            key: 2 * val
                            for key, val in peft_config.rank_pattern.items()
                        }
                    if peft_config.alpha_pattern:
                        peft_config.alpha_pattern = {
                            key: 2 * val
                            for key, val in peft_config.alpha_pattern.items()
                        }
        ...
        **kwargs: Any,
        ...
        if hasattr(config, "runtime_config"):
            ...
            if ephemeral_gpu_offload:
                warnings.warn(
                    "Ephemeral GPU offloading is not supported for this model. Ignoring."
                )
        ...
        if hasattr(model, "hf_device_map"):
            ...
            for name, module in model.named_modules():
                if hasattr(module, "_hf_hook") and hasattr(
                    module._hf_hook, "original_devices"
                ):
                    ...
                    for key in module._hf_hook.original_devices.keys():
                        if module._hf_hook.original_devices[key] == torch.device(
                            "meta"
                        ):
            ...
            if disk_modules and not kwargs.get("use_safetensors", True):
                raise ValueError(
                    "Disk offloading currently only supported for safetensors"
                )
        ...
        if isinstance(getattr(model, "base_model", None), XLoraModel):
            if not isinstance(config, XLoraConfig):
                raise TypeError(
                    f"Expected 'XLoraConfig', got '{type(config)}' instead."
                )
            if "adapters" in kwargs:
                ...
                if not os.path.exists(model_id):
                    ...
                    adapter_names = [
                        file["name"][len(model_id) + 1 :]
                        for file in s.ls(model_id)
                    ...
                    if "adapters" not in kwargs:
                        raise ValueError(
                            "If model_id is a local path, then `adapters` must be passed in kwargs."
                        )
    ...
    def _update_offload(
        self,
        offload_index: dict[str, dict[str, str]],
        ...
        for adapter_name in adapter_names:
            ...
            for key in keys:
                ...
                if isinstance(module, BaseTunerLayer):
                    new_key = (
                        prefix + key[:suffix_pos] + ".base_layer" + key[suffix_pos:]
                    )
                else:
                    new_key = prefix + key
                ...
                with safe_open(fname, framework="pt") as f:
                    for safe_key in f.keys():
                        ...
                        if isinstance(safe_module, BaseTunerLayer):
                            final_key = (
                                extended_prefix + ".base_layer" + safe_key[suffix_pos:]
                            )
                            lora_dict = {
                                key: val
                                for key, val in adapters_weights.items()
                                if extended_prefix in key
                            }
    ...
    def prepare_inputs_for_generation(
        ...
        if peft_config.is_prompt_learning:
            ...
            requires_prompt_injection = (model_kwargs["past_key_values"] is None) or (
                ...
                and not model_kwargs["past_key_values"]
            )
...
def get_layer_status(model: torch.nn.Module) -> list[TunerLayerStatus]:
    ...
    for name, module in base_model.named_modules():
        ...
        mapping_requires_grad_list: dict[str, list[bool]] = collections.defaultdict(
            list
        )
        ...
        requires_grad = {
            key: check_irrgular(vals)
            for key, vals in mapping_requires_grad_list.items()
        }

        devices_dd = collections.defaultdict(list)
        for adapter_module_name in (
            module.adapter_layer_names + module.other_param_names
        ):
            adapter_module = getattr(module, adapter_module_name)
            if isinstance(adapter_module, torch.nn.ModuleDict):
                for key, submodule in adapter_module.items():
                    devices_dd[key].extend(
                        [param.device.type for param in submodule.parameters()]
                    )
...
def get_model_status(model: torch.nn.Module) -> TunerModelStatus:
    ...
    if isinstance(model, PeftModel):
        ...
        base_model = model.base_model
        peft_types = {
            key: str(config.peft_type).partition(".")[-1]
            for key, config in base_model.peft_config.items()
        }
    ...
    enabled_set: set[bool] = {
        status.enabled for status in layer_status
    ...
    available_adapters: list[str] = sorted(
        set().union(*(status.available_adapters for status in layer_status))
    )

    # ideally, active adapters should be consistent across all layers of the model, but we cannot guarantee it
    all_active_adapters: set[tuple[str, ...]] = {
        tuple(status.active_adapters) for status in layer_status
    }
    active_adapters: list[str] | Literal["irregular"]
    ...
    requires_grad_all: dict[str, list[bool | Literal["irregular"]]] = (
    ...
    def check_irrgular(
        vals: list[bool | Literal["irregular"]],
    ) -> bool | Literal["irregular"]:
    ...
    requires_grad = {
        key: check_irrgular(vals) for key, vals in requires_grad_all.items()
    }
