...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--use_oft",
        action="store_true",
        help="Whether to use OFT for parameter efficient tuning",
    )
    parser.add_argument(
        "--oft_r", type=int, default=8, help="OFT rank, only used if use_oft is True"
    )
    parser.add_argument(
        "--oft_alpha",
        type=int,
        default=32,
        help="OFT alpha, only used if use_oft is True",
    )
    parser.add_argument(
        "--oft_dropout",
        type=float,
        default=0.0,
        help="OFT dropout, only used if use_oft is True",
    )
    parser.add_argument(
        "--oft_use_coft",
        action="store_true",
        help="Using constrained OFT, only used if use_oft is True",
    )
    parser.add_argument(
        "--oft_eps",
        type=float,
