...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to peft trained LoRA",
    )
    ...
    if os.path.exists(text_encoder_peft_lora_path):
        text_encoder = CLIPTextModel.from_pretrained(
            ...
            revision=args.sd_checkpoint_revision,
        )
        ...
        kohya_ss_state_dict.update(
            get_module_kohya_state_dict(
                text_encoder, LORA_PREFIX_TEXT_ENCODER, dtype, LORA_ADAPTER_NAME
            )
        )
    ...
    if os.path.exists(unet_peft_lora_path):
        ...
        unet = PeftModel.from_pretrained(
            unet, unet_peft_lora_path, adapter_name=LORA_ADAPTER_NAME
        )
        kohya_ss_state_dict.update(
            get_module_kohya_state_dict(
                unet, LORA_PREFIX_UNET, dtype, LORA_ADAPTER_NAME
            )
        )
