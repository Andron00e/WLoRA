...
from utils.args_loader import (import_model_class_from_model_name_or_path,
                               parse_args)
...
def main(args):
    ...
    if args.enable_xformers_memory_efficient_attention:
        if is_xformers_available():
            ...
            if args.train_text_encoder and not (
    ...
    params_to_optimize = [
        param for param in controlnet.parameters() if param.requires_grad
    ]
    ...
    if args.resume_from_checkpoint:
        if args.resume_from_checkpoint != "latest":
            ...
            if "checkpoint-current" in dirs:
                ...
                dirs = [
                    d for d in dirs if d.startswith("checkpoint") and d.endswith("0")
                ]
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(controlnet), accelerator.accumulate(unet):
                    ...
                    if accelerator.sync_gradients:
                        params_to_clip = (
                            itertools.chain(
                                controlnet.parameters(), text_encoder.parameters()
                            )
                            if args.train_text_encoder
                ...
                if accelerator.sync_gradients:
                    ...
                    if accelerator.is_main_process:
                        ...
                        if global_step % args.checkpointing_steps == 0:
                            ...
                            logger.info(
                                f"Saved {global_step} state to {step_save_path}"
                            )
